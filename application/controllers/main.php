...
class Main extends CI_Controller {
	public function index()
	{
        ...
        public function empHomepage($UserId){              
               ...
               if($this->session->userdata('isLoggedIn')==='1'){    
                    ...
                    $data['cexperiance']= $this->model_users->get_current_experiance($UserId);                  
        ...
        public function viewEmployees(){
            $this->load->library('session'); 
               ...
               if($this->session->userdata('isLoggedIn')==='1'){ 
                    ...
                    $data['num'] = $this->model_users->count_notificiaton();
                    $data['projects']=  $this->model_users->get_linemanager_projects();
                    $data['allemployees']=  $this->model_users->get_all_employees();
        ...
        public function uploaddd_image(){
            $this->load->library('session');
           $this->load->model('model_users');
           
           $this->model_users->upload_image();
          // $this->load->view('upload');
         $userid = $this->model_users->get_user_id();
            $this->empHomepage($userid);
          

         //  redirect('/index.php/main/resetPassword/','refresh');
        ...
        public function change_password(){
            ...
            if($this->model_users->change_password()){
        ...
        public function request_to_get(){            
         ...
         public function request_to_get1(){            
            $this->load->model('model_users');
            $this->model_users->request_to_getemp1();
            $this->employeePool();
        }
     ...
     public function in_the_pool(){
            $this->load->model('model_users');
            $this->model_users->in_the_pool($this->input->post('UserId'));        
     }

     public function send_email(){
        $this->load->model('model_users');
        $data = $this->model_users->send_email($this->input->post('email'));
        if( $data == 'success'){
            $data1['h1title'] = 'Successful Registration';
            $data1['subtext'] = '<p>Test to go here</p>';
    ...
    // Load the message page
        ...
        public function customsearch(){
            $this->load->library('session');
            $this->load->view('customsearch');
        }
        public function search(){
               $this->load->model('model_users');
               $this->model_users->search();
        }
